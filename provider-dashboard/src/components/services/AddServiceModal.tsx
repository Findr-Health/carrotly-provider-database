...
export function AddServiceModal({ onClose, onSave }: AddServiceModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validate()) {
      onSave({ ...formData, price: Math.round(formData.price * 100) });
      onClose();
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-teal-50 border border-teal-200 rounded-lg p-4">
            <h3 className="font-semibold text-teal-900 mb-2">Preview:</h3>
            <div className="text-sm text-teal-800 space-y-1">
              <p><strong>Service:</strong> {formData.name || 'Service name'}</p>
