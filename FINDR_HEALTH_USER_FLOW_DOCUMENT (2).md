...
## Appendix D: Future Screens (Not Yet Designed)

The following features are planned for future releases but are not included in the current scope. Placeholder UI elements (tiles, menu items) may exist in the current design.

### D.1 Payments & Financial

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-PAY-001` | Pay Bill | View outstanding balance, payment history, make payment | Medium | Billing system integration |
| `FUT-PAY-002` | Payment History | List of all transactions with filtering | Medium | Transaction logging |
| `FUT-PAY-003` | Invoice Detail | Detailed receipt/invoice view with download option | Medium | Invoice generation |
| `FUT-PAY-004` | HSA/FSA Integration | Connect health savings accounts for payment | Low | HSA provider APIs |

### D.2 Rewards & Loyalty

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-REW-001` | Rewards Hub | Points balance, tier status, available rewards | Medium | Loyalty program backend |
| `FUT-REW-002` | Earn Points | How to earn points (bookings, reviews, referrals) | Medium | Points calculation logic |
| `FUT-REW-003` | Redeem Rewards | Available rewards catalog, redemption flow | Medium | Rewards inventory |
| `FUT-REW-004` | Rewards History | Points earned/spent history | Low | Points transaction log |

### D.3 Insurance Features (Enhanced)

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-INS-001` | Insurance Verification | Real-time eligibility check | High | Insurance verification API |
| `FUT-INS-002` | Coverage Estimator | Estimate out-of-pocket based on insurance + service | High | Benefits data integration |
| `FUT-INS-003` | Claim Status | Track submitted claims | Medium | Claims processing system |
| `FUT-INS-004` | EOB Viewer | View Explanation of Benefits documents | Low | Document storage |

### D.4 Telehealth

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-TELE-001` | Video Call Lobby | Pre-call device check, waiting room | High | WebRTC/video SDK |
| `FUT-TELE-002` | Video Call Active | Full-screen video with controls (mute, camera, end) | High | Video infrastructure |
| `FUT-TELE-003` | Chat Consultation | Text-based async consultation with provider | Medium | Secure messaging |
| `FUT-TELE-004` | Post-Visit Summary | Notes, prescriptions, follow-up from telehealth visit | Medium | EHR integration |

### D.5 Health Records

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-REC-001` | Health Records Hub | Central view of all health documents | Medium | Document storage |
| `FUT-REC-002` | Upload Document | Upload lab results, prescriptions, records | Medium | File upload system |
| `FUT-REC-003` | Share Records | Share records securely with providers | Low | Secure sharing |
| `FUT-REC-004` | Medical History Form | Digital intake form for new providers | Medium | Form builder |

### D.6 Family & Dependents

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-FAM-001` | Family Members | List of dependents/family members | Medium | Multi-profile system |
| `FUT-FAM-002` | Add Family Member | Add dependent with relationship, DOB | Medium | Profile management |
| `FUT-FAM-003` | Book for Family | Select which family member appointment is for | Medium | Booking system update |
| `FUT-FAM-004` | Family Bookings | View all family appointments in one place | Low | Query optimization |

### D.7 Enhanced Support

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-SUP-001` | Live Chat Support | Real-time chat with support agent | Medium | Chat infrastructure |
| `FUT-SUP-002` | Support Ticket Detail | View status and updates on submitted tickets | Low | Ticketing system |
| `FUT-SUP-003` | Schedule Callback | Request phone callback from support | Low | Call center integration |

### D.8 Accessibility & Personalization

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-ACC-001` | Accessibility Settings | Font size, high contrast, screen reader hints | Medium | Accessibility audit |
| `FUT-ACC-002` | Language Selection | Multi-language support | Medium | i18n implementation |
| `FUT-ACC-003` | Health Preferences | Dietary restrictions, accessibility needs for providers | Low | Provider matching |

### D.9 Provider Communication

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-MSG-001` | Messages Inbox | Direct messages with providers | Medium | Messaging system |
| `FUT-MSG-002` | Conversation Thread | Chat thread with specific provider | Medium | Real-time messaging |
| `FUT-MSG-003` | Message Attachments | Send/receive images, documents | Low | File handling |

### D.10 Analytics & Insights

| Screen ID | Screen Name | Description | Priority | Dependencies |
|-----------|-------------|-------------|----------|--------------|
| `FUT-ANL-001` | Spending Summary | Healthcare spending over time, by category | Low | Analytics processing |
| `FUT-ANL-002` | Visit History | Timeline of all healthcare visits | Low | Data aggregation |
| `FUT-ANL-003` | Health Goals | Set and track wellness goals | Low | Goal tracking system |

---

## Appendix E: Technical Debt & Known Limitations

| Item | Description | Impact | Resolution Plan |
|------|-------------|--------|-----------------|
| PIN vs Biometric | PIN confirmation is placeholder; should migrate to biometric for production | Security UX | Phase 5 biometric implementation |
| Offline Mode | No offline capability currently | Poor experience in low connectivity | Future: cache critical data locally |
| Real-time Availability | Availability is fetched, not real-time | Possible double-booking race condition | Future: WebSocket for live availability |
| Search Performance | Full-text search on MongoDB | May slow at scale | Future: Elasticsearch integration |
| Image Optimization | No CDN or image compression pipeline | Slow load times | Future: Cloudinary or similar |
