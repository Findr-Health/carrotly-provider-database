// New Service Templates for Labs, Rapid Tests, IV Therapy, Immunizations, etc.
// Run with: node seed_new_templates.js

const mongoose = require('mongoose');

const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://findrhealth:uOgCvSMAqG0W2bAR@findrhealth.aybud1j.mongodb.net/findrhealth?retryWrites=true&w=majority&appName=FindrHealth';
...
const serviceTemplateSchema = new mongoose.Schema({
  ...
  suggestedVariants: [{ name: String, priceModifier: Number, durationModifier: Number, description: String }]
}, { timestamps: true });
...
const newTemplates = [
  ...
  { providerType: 'Medical', category: 'Wellness', name: "Men's Health Panel", description: 'Comprehensive male health screening including PSA and testosterone.', shortDescription: "Men's screening", suggestedPriceMin: 150, suggestedPriceMax: 275, suggestedDuration: 30, isPopular: false, sortOrder: 2 },
  { providerType: 'Medical', category: 'Wellness', name: "Women's Health Panel", description: 'Comprehensive female health screening including hormone levels.', shortDescription: "Women's screening", suggestedPriceMin: 150, suggestedPriceMax: 300, suggestedDuration: 30, isPopular: false, sortOrder: 3 },

  // ============================================
  // SKINCARE - IV Therapy (Med Spas often offer this)
  // ============================================
...
async function seedTemplates() {
  try {
    ...
    for (const template of newTemplates) {
      ...
      added++;
      console.log(`Added: ${template.providerType} > ${template.category} > ${template.name}`);
    }
    
    console.log(`\nComplete! Added: ${added}, Skipped (already exists): ${skipped}`);
    
    // Show category counts
    const categories = await ServiceTemplate.aggregate([
      { $group: { _id: { providerType: '$providerType', category: '$category' }, count: { $sum: 1 } } },
      { $sort: { '_id.providerType': 1, '_id.category': 1 } }
    ]);
    
    console.log('\n=== Template Categories ===');
    let currentType = '';
    for (const cat of categories) {
      if (cat._id.providerType !== currentType) {
        currentType = cat._id.providerType;
        console.log(`\n${currentType}:`);
      }
      console.log(`  ${cat._id.category}: ${cat.count} templates`);
    }
