...
const sendProviderInvite = async ({ toEmail, providerName, providerType, city, state }) => {
  ...
  const html = `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;"><div style="max-width:600px;margin:0 auto;"><div style="background:linear-gradient(135deg,#0d9488,#14b8a6);color:white;padding:30px;text-align:center;border-radius:10px 10px 0 0;"><h1>Findr Health</h1></div><div style="background:#f9fafb;padding:30px;border:1px solid #e5e7eb;"><p>${greeting},</p><div style="background:#d1fae5;padding:15px;border-radius:8px;margin:20px 0;border-left:4px solid #10b981;"><strong>Patients in ${location} are actively searching for ${providerType} providers on Findr Health!</strong></div><p>We'd love to connect them with your practice.</p><h3>Why Join?</h3><ul><li>Free Listing</li><li>Transparent Pricing</li><li>Direct Bookings</li><li>Analytics Dashboard</li></ul><p><a href="https://providers.findrhealth.com" style="display:inline-block;background:#0d9488;color:white;padding:14px 28px;text-decoration:none;border-radius:8px;">Join Findr Health</a></p><p>Best regards,<br>The Findr Health Team</p></div></div></body></html>`;
  
  return sendEmail({ to: toEmail, subject, text: `${greeting}, Patients in ${location} are looking for ${providerType} providers. Join: https://providers.findrhealth.com`, html });
};
