...
## Findr Health - Detailed Roadmap (Updated)
...
1. Users can cancel bookings with automatic refunds per policy
...
5. Small providers have basic scheduling tool option

### **Technical Goals:**
1. Bidirectional calendar sync (create, update, delete)
2. Automated notification system
3. Proper refund handling via Stripe (existing policies)
4. Provider dashboard for booking management
5. Basic Google Calendar scheduling platform
...
5. Offer simple scheduling alternative for small providers
...
#### **User Stories:**
1. As a **patient**, I want to cancel my booking per the applicable cancellation policy
...
**Backend Changes:**
1. Cancel booking endpoint with policy-based refund logic
...
2. Confirmation dialog showing applicable refund amount
...
```json
{
  ...
  "booking": {
    ...
    "refund": {
      ...
      "status": "processed",
      "refundId": "re_abc123",
      "policy": "Standard - 24+ hours before"
    },
    "calendar": {
      "eventDeleted": true
    }
...
**Step 1: Backend - Policy-Based Refund Logic (3 days)**
```javascript
...
router.post('/:id/cancel', authenticateUser, async (req, res) => {
  ...
  const policy = booking.provider.cancellationPolicy || 'standard';
  
  // Calculate refund based on provider's selected policy
  const { refundAmount, refundPercentage, policyDescription } = 
    calculateRefundByPolicy(booking, policy);
  
  // Process Stripe refund (Stripe handles transfer timing automatically)
...
// Helper function - Policy-based refund calculation
function calculateRefundByPolicy(booking, policy) {
  const now = new Date();
  ...
  if (policy === 'standard') {
    // Standard Policy (from existing agreements)
    if (hoursUntilAppointment >= 24) {
      refundPercentage = 100;
      policyDescription = 'Standard - 24+ hours before';
    } else if (hoursUntilAppointment >= 12) {
      refundPercentage = 75;
      policyDescription = 'Standard - 12-24 hours before (25% fee)';
    } else if (hoursUntilAppointment > 0) {
      refundPercentage = 50;
      policyDescription = 'Standard - Less than 12 hours (50% fee)';
    } else {
      refundPercentage = 0;
      policyDescription = 'Standard - No-show (100% fee)';
    }
  } else if (policy === 'moderate') {
    // Moderate Policy (from existing agreements)
    if (hoursUntilAppointment >= 48) {
      refundPercentage = 100;
      policyDescription = 'Moderate - 48+ hours before';
    } else if (hoursUntilAppointment >= 24) {
      refundPercentage = 75;
      policyDescription = 'Moderate - 24-48 hours before (25% fee)';
    } else if (hoursUntilAppointment > 0) {
      refundPercentage = 50;
      policyDescription = 'Moderate - Less than 24 hours (50% fee)';
    } else {
      refundPercentage = 0;
      policyDescription = 'Moderate - No-show (100% fee)';
    }
  }
  
  const refundAmount = (booking.payment.originalAmount * refundPercentage) / 100;
  
  return { refundAmount, refundPercentage, policyDescription };
}
```

**Note on Stripe Transfers:**
- Stripe automatically handles payout timing based on provider's transfer schedule
- No manual transfer logic needed - Stripe's defaults apply
- Providers configure their payout preferences in Stripe Dashboard
- Typical: 2-day rolling basis or weekly schedule
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: refundInfo.percentage >= 75 
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Refund: \$${refundInfo.amount.toStringAsFixed(2)} (${refundInfo.percentage}%)',
                  style: TextStyle(
                ...
                if (refundInfo.percentage < 100) ...[
                  ...
                  Text(
                    'Fee: \$${refundInfo.fee.toStringAsFixed(2)}',
                    style: TextStyle(
                      fontSize: 14,
  ...
  try {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Booking cancelled - \$${_calculateRefundInfo(booking).amount.toStringAsFixed(2)} refund processing'),
          backgroundColor: Colors.green,
        ),
      );
      context.pop(); // Return to bookings list
    }
  } catch (e) {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
...
- [ ] Standard policy: 24+ hrs → 100% refund
- [ ] Standard policy: 12-24 hrs → 75% refund
- [ ] Standard policy: <12 hrs → 50% refund
- [ ] Moderate policy: 48+ hrs → 100% refund
- [ ] Moderate policy: 24-48 hrs → 75% refund
- [ ] Moderate policy: <24 hrs → 50% refund
...
- [ ] UI shows correct refund amount based on policy
- [ ] Provider cancellation → always 100% refund
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'Remaining reschedules: ${widget.currentBooking.reschedule.maxAttempts - widget.currentBooking.reschedule.count - 1}',
              style: const TextStyle(fontSize: 12, color: Colors.grey),
            ),
          ],
        ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    _formatDate(widget.currentBooking.dateTime.requestedStart),
                    style: const TextStyle(fontWeight: FontWeight.w500),
                  ),
                  Text(
                    _formatTime(widget.currentBooking.dateTime.requestedStart),
                    style: const TextStyle(fontWeight: FontWeight.w500),
                  ),
                ],
              ),
  ...
  Future<void> _processReschedule() async {
    ...
    try {
      ...
      await repository.rescheduleBooking(
        bookingId: widget.currentBooking.id,
        newStartTime: _combineDateAndTime(_newDate!, _newTimeSlot!),
        reason: 'User requested',
      );
      ...
      if (mounted) {
        ...
        context.pop(); // Return to booking details
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
...
async function sendBookingConfirmation(booking, user, provider) {
  const msg = {
    ...
    dynamicTemplateData: {
      ...
      cancellationPolicy: provider.cancellationPolicy || 'standard'
    }
  };
...
### **Feature 5: Basic Scheduling Platform** 📅
...
#### **User Stories:**
1. As a **small provider**, I want a simple scheduling tool if I don't have one
2. As a **provider**, I want it integrated with my Google Calendar
3. As a **provider**, I don't want complex office management features

#### **Technical Requirements:**

**What It Is:**
- Simple Google Calendar-based scheduling interface
- Display availability from Google Calendar
- Book appointments directly to Google Calendar
- Basic appointment management
- Integration with Findr Health bookings

**What It's NOT:**
- Full office management system
- EHR integration
- Billing/invoicing system
- Patient records management
- Replacement for robust existing systems

#### **Implementation Steps:**

**Step 1: Calendar View Component (1 week)**
```javascript
// Frontend: Simple week/month calendar view
// Shows availability from provider's Google Calendar
// Click to book → creates Findr booking + calendar event
// Drag to reschedule → updates both systems
```

**Step 2: Google Calendar Integration (1 week)**
```javascript
// Already have OAuth flow from Phase 1
// Extend to support:
// - Read all events (for availability display)
// - Create events (booking from scheduling tool)
// - Update events (reschedule)
// - Delete events (cancellation)
```

**Step 3: Basic Appointment Management (1 week)**
```javascript
// Simple list view of appointments
// Filter by date, status
// Quick actions: confirm, cancel, reschedule
// Patient contact info display
// Notes field for provider
```

**Design Philosophy:**
- **Google Calendar is source of truth** for availability
- **Findr Health** handles bookings, payments, notifications
- **Scheduling platform** is just a UI layer over both
- **Keep it simple** - calendar view + appointment list + quick actions
- **Target audience:** Solo practitioners, small clinics (1-5 providers)

**Step 4: Testing (3 days)**
- [ ] Calendar displays Google Calendar availability
- [ ] Click to book creates Findr booking
- [ ] Findr bookings appear in calendar view
- [ ] Reschedule updates both systems
- [ ] Cancellation removes from both systems
- [ ] Works for providers without existing scheduling tools
- [ ] Doesn't confuse providers with robust existing systems
...
- Days 1-3: Backend policy-based refund logic
...
### **Week 7-8: Basic Scheduling Platform** (Optional/Parallel)
- Week 7: Calendar view + Google integration
- Week 8: Appointment management + testing
...
### **Development Team:**
- 1 Backend Engineer (full-time, 6-8 weeks)
- 1 Frontend Engineer (full-time, 6-8 weeks)
...
| Risk | Probability | Impact | Mitigation |
|------|-------------|--------|------------|
| User abuse of refunds | Low | Medium | Policies already documented |
...
- [ ] Scheduling platform guide (for small providers)
...
**Version:** 2.0 (Updated with existing policies, scheduling platform, no EHR)  
**Approved By:** [Pending]  
**Start Date:** [TBD]  
**Estimated Completion:** [Start Date + 6-8 weeks]
