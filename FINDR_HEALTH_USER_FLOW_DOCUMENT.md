...
| `AUTH-002` | Sign In | Email/password fields, social auth (Facebook, Google, Apple), "Remember me" toggle, "Forgot password" link | Splash (unauthenticated), Sign Up link, Logout |
...
| Screen ID | Screen Name | Description | Entry Points |
|-----------|-------------|-------------|--------------|
| `HOME-001` | Home | Greeting, search bar (voice enabled), "Your Next Appointment" card, horizontal provider sections (Viewed, Our Picks, What's Popular, Just Added), "Explore by Type" category grid, floating Clarity button | Bottom nav Tab 1 |
| `SRCH-001` | Search Keyword | Search input (focused), recent searches, top categories with icons | Tap search bar on Home |
...
| `PROV-001` | Provider Detail | Hero image gallery, rating, address, hours, service tabs with pricing, team avatars, reviews preview, "About us" | Tap provider card anywhere |
...
| Screen ID | Screen Name | Description | Entry Points |
|-----------|-------------|-------------|--------------|
| `BOOK-001` | Select Team Member | Optional step - list of available professionals with photos, ratings | Tap "Book" on service (if provider has team) |
| `BOOK-002` | Select Date & Time | Calendar picker, time slot chips (30-min intervals), availability from provider calendar | After service/team selection |
| `BOOK-003` | Payment Methods | Select existing card (PayPal, Google Pay, Apple Pay, saved cards) or add new | After date/time confirmed |
| `BOOK-004` | Add New Card | Card visual preview, form fields (number, name, expiry, CVV), "Add" button | Tap "Add New Card" on Payment Methods |
| `BOOK-005` | Review Summary | Provider info, service, date/time, duration, price breakdown (subtotal + admin fee + total), "Confirm" button | After payment method selected |
| `BOOK-006` | Enter PIN | 4-digit PIN pad for booking confirmation | Tap "Confirm" on Review Summary |
...
| `BOOK-008` | Booking Success | Teal checkmark illustration, confirmation message, "View Appointment" / "Cancel" buttons | Booking completed successfully |
...
| `MYBK-002` | Booking Detail | Full appointment info, provider details, actions (Cancel, Reschedule, Get Directions) | Tap booking card |
...
| `MYBK-005` | Reschedule Success | Success illustration, "View Appointment" / "Cancel" buttons | Reschedule confirmed |
| `MYBK-006` | Cancel - Confirmation Modal | Overlay showing refund amount (based on cancellation policy), "Back" / "Yes, Cancel" buttons | Tap "Cancel" on booking |
| `MYBK-007` | Cancel - Reason | Radio button list of reasons + "Others" textarea | Confirm cancellation |
| `MYBK-008` | Cancel Success | Success illustration with "OK" button | Cancellation processed |
...
| `PROF-002` | Edit Profile | Form fields: name, DOB (date picker), email, country dropdown, phone with country code, gender dropdown, "Update" button | Profile "Edit Profile" tile |
| `PAY-001` | Payment Methods | List of saved cards (visual previews), "Add New Card" button | Profile "Payment + Insurance" tile |
...
| `SET-001` | Settings | Grouped list: Account, Preferences, Support, Legal, Danger Zone | Profile gear icon |
| `SET-002` | Notifications Settings | Toggles for push, email, SMS by category (bookings, reminders, promotions) | Settings "Notifications" |
...
| `SET-005` | Help Center | FAQ accordion or searchable help articles | Settings "Help Center" |
| `SET-006` | Report a Problem | Category dropdown, description textarea, screenshot attach option | Settings "Report a Problem" |
| `SET-007` | Delete Account | Warning message, password confirmation, "Delete" button | Settings "Delete Account" |
...
| `UTIL-002` | Force Update | App update illustration, version info, "Update Now" button (links to store) | App launch when version < minimum |
| `UTIL-003` | Soft Update Prompt | Update available banner/modal, "Update" / "Later" buttons | App launch when update available |
| `UTIL-004` | Guest Login Prompt | Modal explaining login required to book, "Sign In" / "Create Account" / "Cancel" | Tap "Book" as guest |
...
└────────┬────────┘
         │
         ├─── Email/Password ───► Validate ───► HOME-001
         │
...
| Booking Cancelled | `MYBK-001` (Cancelled tab) |
| Schedule Changed | `MYBK-002` (specific booking) |
...
| BOOK-005 | Create Booking | POST | `/api/bookings` | `{ providerId, serviceId, teamMemberId, date, time, paymentMethodId }` | `{ booking, clientSecret }` |
...
1. Auth flow (Sign In, Sign Up, Forgot Password)
2. Home screen with provider sections
3. Search with map + list
4. Provider detail with services
5. Basic booking flow (no team selection)
6. My Bookings list
